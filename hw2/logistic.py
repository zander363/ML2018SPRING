...
"""
this script is use logistic regression
to train the model
"""
import os, sys
import numpy as np
...
def train(X_all, Y_all, save_dir):
    ...
    valid_set_percentage = 0.38
    X_train, Y_train, X_valid, Y_valid = split_valid_set(X_all, Y_all, valid_set_percentage)

    #X_train = np.concatenate((X_train,np.log((X_train[:,78]+1e-10).reshape(-1,1))),axis=1)
    #X_valid = np.concatenate((X_valid,np.log((X_valid[:,78]+1e-10).reshape(-1,1))),axis=1)
    X_train = np.concatenate((X_train,(X_train[:,78].reshape(-1,1))**2),axis=1)
    X_valid = np.concatenate((X_valid,(X_valid[:,78].reshape(-1,1))**2),axis=1)
    ...
    for epoch in range(1, epoch_num):
        ...
        #X_train = np.concatenate((X_train,np.log(X_train[:,78]).reshape((-1,1))),axis=1)
        #X_train[:,78] = np.log(X_train[:,78])
        ...
        for idx in range(step_num):
            ...
            w = w - l_rate * (w_grad + 2*lambda_c*w)
            b = b - l_rate * b_grad

    return


def infer(X_test, save_dir, output):
    test_data_size = len(X_test)
    X_test = np.concatenate((X_test,(X_test[:,78].reshape(-1,1))**2),axis=1)
    ...
    print('=====Write output to %s =====' % output)
    '''
...
def main(opts):
    ...
    if opts.train:
        ...
        infer(X_test, opts.save_dir, opts.output)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_data_path', type=str,
                        default='feature/train_X', dest='train_data_path',
                        help='Path to training data')
    parser.add_argument('--train_label_path', type=str,
                        default='feature/train_Y', dest='train_label_path',
                        help='Path to training data\'s label')
    parser.add_argument('--test_data_path', type=str,
                        default='feature/test_X', dest='test_data_path',
                        help='Path to testing data')
    parser.add_argument('--save_dir', type=str,
                        default='logistic_params/', dest='save_dir',
                        help='Path to save the model parameters')
    parser.add_argument('--output', type=str,
                        default='logistic_output/log_prediction.csv', dest='output',
                        help='Path to save the model parameters')
